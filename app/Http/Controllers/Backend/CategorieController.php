...

{
    ...
    //stcker la categorie dans la db
    public function store(Request $request)
    {
        $request->validate(['nom=>required | max:255']);
        $categorie = new Categorie();
        $categorie->nom = $request->nom; $categorie->parent_id = $request->parent_id;
        ...
        return redirect()->route('backend_tags_index')->with('notice', 'La categorie <strong>' . $categorie->nom . '</strong> a bien été ajoutée');
    }
    ...
    public function update(Request $request){
        ...
        if ($request->is_online == 1) {
            ...
            return redirect()->route('backend_tags_index')
                ->with('notice', 'La categorie <strong>' . $categorie->nom . '</strong> a été modifiée');
        ...
        $categorie->delete();
        return redirect()->route('backend_tags_index')
            ->with('notice','La categorie <strong>'.$categorie->nom.'</strong> a été supprimée');
